...
add_action( 'wp_enqueue_scripts', 'pmpromd_register_styles', 10 );

function pmpromd_extra_page_settings( $pages ) {
	$pages['directory'] = array(
		'title'   => __( 'Directory', 'pmpro-membership-directory' ),
		'content' => '[pmpro_member_directory]',
		'hint'    => __( 'Include the shortcode [pmpro_member_directory].', 'pmpro-membership-directory' ),
	);
	$pages['profile']   = array(
		'title'   => __( 'Profile', 'pmpro-membership-directory' ),
		'content' => '[pmpro_member_profile]',
		'hint'    => __( 'Include the shortcode [pmpro_member_profile].', 'pmpro-membership-directory' ),
	);
	
	return $pages;
}

add_action( 'pmpro_extra_page_settings', 'pmpromd_extra_page_settings', 10 );
        ...
        <tr class="user-hide-directory-wrap">
            ...
            <td>
                <label for="hide_directory">
                    <input name="hide_directory" type="checkbox" id="hide_directory" <?php checked( get_user_meta( $user->ID, 'pmpromd_hide_directory', true ), 1 ); ?> value="1"><?php printf( __( 'Hide from %s?', 'pmpro-membership-directory' ), get_the_title( $pmpro_pages['directory'] ) ); ?>
                </label>
            </td>
        </tr>
        </tbody>
...
add_action( 'show_user_profile', 'pmpromd_show_extra_profile_fields', 10 );
add_action( 'edit_user_profile', 'pmpromd_show_extra_profile_fields', 10 );
...
add_action( 'personal_options_update', 'pmpromd_save_extra_profile_fields', 10 );
add_action( 'edit_user_profile_update', 'pmpromd_save_extra_profile_fields', 10 );
